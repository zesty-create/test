...
do
    ...
    Library.Watermark.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 then
            Library._WatermarkMoved = true;
        end;
    end);
...
function Library:SetWatermark(Text)
    ...
    local wmWidth = X + 15;
    local wmHeight = (Y * 1.5) + 3;
    Library.Watermark.Size = UDim2.new(0, wmWidth, 0, wmHeight);

    -- центрируем по горизонтали только если пользователь не двигал watermark
    if not Library._WatermarkMoved then
        local screenWidth = ScreenGui.AbsoluteSize.X;
        Library.Watermark.Position = UDim2.fromOffset((screenWidth - wmWidth) / 2, -18);
    end

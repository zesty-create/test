...
function SaveManager:Load(name)
    if not name or not isfile(self.Folder .. '/settings/' .. name .. '.json') then
        return self.Library:Notify('Config not found', 3)
    end

    local content = readfile(self.Folder .. '/settings/' .. name .. '.json')
...
function SaveManager:LoadAutoloadConfig()
    local autoload = isfile(self.Folder .. '/settings/autoload.txt') and readfile(self.Folder .. '/settings/autoload.txt')
